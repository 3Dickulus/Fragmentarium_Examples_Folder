...
float getOpacity(int layerIndex);
...
#define NO_MORE_LAYERS 1000.0			// parent DE() returns this to terminate layer list
#define NO_DATA_FOR_THIS_LAYER -1.0		// DE() walkedmaxSteps without hitting object

#group Layers

uniform vec3 Depth_1_3; slider[(-10,-10,-10),(0,0,0),(10,10,10)]
uniform vec3 Depth_4_6; slider[(-10,-10,-10),(0,0,0),(10,10,10)]
uniform vec3 Depth_7_9; slider[(-10,-10,-10),(0,0,0),(10,10,10)]
uniform bool useActalDepth; checkbox[true]

uniform vec3 Opacity_1_3; slider[(0,0,0),(1,1,1),(1,1,1)]
uniform vec3 Opacity_4_6; slider[(0,0,0),(1,1,1),(1,1,1)]
uniform vec3 Opacity_7_9; slider[(0,0,0),(1,1,1),(1,1,1)]

float getDepth(int index) {
	switch(index) {
		case 0 : return Depth_1_3.x;
		case 1 : return Depth_1_3.y;
		case 2 : return Depth_1_3.z;
		case 3 : return Depth_4_6.x;
		case 4 : return Depth_4_6.y;
		case 5 : return Depth_4_6.z;
		case 6 : return Depth_7_9.x;
		case 7 : return Depth_7_9.y;
		case 8 : return Depth_7_9.z;
		default: return 0;	
	}
}
...
float getOpacity(int index) {
	switch(index) {
		case 0 : return Opacity_1_3.x;
		case 1 : return Opacity_1_3.y;
		case 2 : return Opacity_1_3.z;
		case 3 : return Opacity_4_6.x;
		case 4 : return Opacity_4_6.y;
		case 5 : return Opacity_4_6.z;
		case 6 : return Opacity_7_9.x;
		case 7 : return Opacity_7_9.y;
		case 8 : return Opacity_7_9.z;
		default: return 0;	
	}
}
...
vec3 traceLayer(vec3 from, vec3 dir, inout vec3 hit, inout vec3 hitNormal, inout float distance) {
	hit = vec3(0.0);
	...
	for (steps=0; steps<MaxRaySteps; steps++) {
		...
		if(dist < 0.0) {		// unused layer
			distance = NO_DATA_FOR_THIS_LAYER;
			return vec3(0.0);
		}
...
vec3 trace(vec3 from, vec3 dir, inout vec3 hit, inout vec3 hitNormal) {
	...
	// collect data on all layers ---------------
	layerCount = 0;

	for(int i=0;i<LAYER_MAX;++i) {
		layerIndex = i;

		if(getOpacity(layerIndex) > 0.0) { // skip transparent layers
			layerColor = traceLayer(from, dir, hit, hitNormal,layerDistance);

			if(layerDistance >= NO_MORE_LAYERS - 0.01) break;

			if(layerDistance > NO_DATA_FOR_THIS_LAYER) {	
			    if(useActalDepth)
					layerData[layerCount].distance = layerDistance;
				else
					layerData[layerCount].distance = getDepth(layerIndex);

				layerData[layerCount].color = layerColor;
				layerData[layerCount].opacity = getOpacity(i);
				++layerCount;
			}
		}
	}
	...
	if(layerCount > 1) {
 		for(;;) {
			...
			for(int i=0;i<layerCount-1;++i) {
				if(layerData[i].distance > layerData[i+1].distance) {
					okay = false;
					LayerData t = layerData[i];
	...
	// mix colors for all layers ------------------
	vec3 color = vec3(0.0);
	vec3 accumulatedOpacity = vec3(1.0);

	for(int i=0;i<layerCount-1;++i) {
		color += layerData[i].color * layerData[i].opacity * accumulatedOpacity;
		accumulatedOpacity = max(vec3(0.0), accumulatedOpacity - layerData[i].opacity);
	}
