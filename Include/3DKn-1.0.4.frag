...
//uniform bool UseTrueFOV; checkbox[true]
//uniform float TrueFOV; slider[0.1,115,179]
...
// vec2 rand(vec2 co){
// #ifdef WANG_HASH
//         return vec2(fract(sin(dot(wang_hash_fp(co) , vec2(12.9898,78.233))) * 43758.5453),
//                 fract(cos(dot(wang_hash_fp(co), vec2(4.898,7.23))) * 23421.631));
// #else
...
void main(void){
	...
	#ifdef USE_EXTERNAL_CAMERA
		if (iCamWeight>0){
			frame = int(ceil(iCamFrame));
