...
uniform int RefineSteps; slider[1,4,20];
...
uniform int MaxRaySteps;  slider[0,56,20000]
uniform float MaxDistance;slider[0,20,10000];
...
uniform vec4 SpotLight; color[0.0,1.,20.0,1.0,1.0,1.0];
uniform vec3 LightPos; slider[(-30,-30,-30),(0,0,0),(30,30,30)]
...
uniform int HF_FogIter; slider[1,1,64]
...
//              2./16777216. = 2./(24bit max) = 0.000000119209289551 rounds to float 0.0000001 ?
#define MIN_EPS 0.000000119209289551
...
vec3 trace(inout SRay Ray, inout vec3 hitNormal, inout float glow) {
	...
	float eps = minDist;
        float lightde=0.;
        float epsModified = max(MIN_EPS, Ray.Pos*eps);
        orbitTrap = vec4(10000.0);
	...
	for (steps=0; steps<MaxRaySteps; steps++) {
		vec3 p = SRCurrentPt(Ray); //from + totalDist * direction;
                ...
                SRAdvance(Ray, dist);
                epsModified = max(MIN_EPS, Ray.Pos*eps);
                if (dist < epsModified) {
			// move back
...
vec4 integrateClouds( in vec4 sum, in float dif, in float den, in float t )
{
    ...
    vec4 col = vec4( mix( vec3(1.0,0.95,0.8), vec3(0.0), den ), den );
    col.xyz *= lin;
...
// forward decl for adding fog anisotropy to clouds
vec3 ptLightGlow3(vec3 P0, vec3 P1);

vec4 clouds(vec3 p0, vec3 p1){
        vec3 ro=p0 + ((WindDir*WindSpeed)*time),rd=normalize(p1-ro),cloudDir=normalize(HF_Dir);
        vec4 sum = vec4(0.0);
        float t=0.1*CloudScale*rand( vec3(gl_FragCoord.xyz*float(subframe+1) )) ,maxT=length(p1-ro);
        bool goingUp=(dot(rd,cloudDir)>0.0);
	while(t<maxT) {
                vec3  pos = ro + t*rd;
                if((!goingUp && dot(pos,cloudDir)<CloudBase) || (goingUp && dot(pos,cloudDir)>CloudTops) || sum.a > 0.9) break;
                float den = cloudDensity(pos,cloudDir,t);
                if( den>0.001 ) {
                        vec3 sundir=normalize(LightPos-pos);
                        float dif = clamp((den - cloudDensity(pos+0.5*CloudScale*sundir,cloudDir,t)), -0.5, 0.5 );
...
#ifdef KN_VOLUMETRIC
for(int j=0;j<HF_FogIter;j++) sum.xyz+=(ptLightGlow3(p0, p1)*(1./float(HF_FogIter)));
#endif
...
vec3 ptLightGlow3(vec3 P0, vec3 P1){
	...
	float PDF_NF=(atanBC-atanB)/Delta;//Normalization factor due to division by the PDF.
        float t=(tan(mix(atanB,atanBC,x))*Delta-B)/C;//when x is close to 0. there are black dots!
        t=clamp(t,0.0001,1.); //This is very strange: I have to clamp this way to get rid from black dots. When doing clamp(t,0.,1.) I get much more black dots.
        vec3 Pt=P0+t*(P1-P0);//this is the sample point
	...
	vec3 Anie=(1.-HF_Anisotropy*HF_Anisotropy)/sqrt(denom*denom*denom);//Dropped the 1./(4.*PI)* factor. Well This renderer is not meant to be physically accurate after all. :o)
        //Scattering at sample point
        vec3 samVal=extP0t*extLt*densiTy*Anie*shadowStrength*length(P1-P0);//sqrt(C) is the length of the path.
        //Return scattered light
	//samVal=max(0.,samVal);
	return SpotLight.xyz*SpotLight.w*PDF_NF*samVal*HF_Scatter;
}
